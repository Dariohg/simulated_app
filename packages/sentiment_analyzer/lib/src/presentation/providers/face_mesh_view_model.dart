...
class FaceMeshViewModel extends ChangeNotifier {
  ...
  int _frameCount = 0;
  final int _processEmotionEveryNFrames = 2;
  ...
  void applyCalibration(CalibrationResult calibration) {
    ...
    debugPrint('[ViewModel] Calibracion aplicada');
    debugPrint('[ViewModel]   EAR threshold: ${calibration.earThreshold}');
    debugPrint('[ViewModel]   Baseline pitch: ${calibration.baselinePitch}');
    debugPrint('[ViewModel]   Baseline yaw: ${calibration.baselineYaw}');
  }
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      debugPrint('[ViewModel] ERROR inicializando camara: $e');
    }
  }
  ...
  void _processFrame(CameraImage image) async {
    ...
    try {
      ...
      debugPrint('[ViewModel] ERROR procesando frame #$_frameCount: $e');
      if (_frameCount <= 5) {
  ...
  Future<EmotionResult?> _processEmotion(
    ...
    try {
      ...
      debugPrint('[ViewModel] ERROR en _processEmotion: $e');
      return null;
    }
  }
  ...
  InputImage? _convertCameraImageToInputImage(CameraImage image) {
    ...
    try {
      ...
      debugPrint('[ViewModel] ERROR convirtiendo a InputImage: $e');
      return null;
    }
  }
  ...
  void _logPeriodicStats() {
    ...
    if (now.difference(_lastStatsLog).inSeconds >= 5) {
      ...
      debugPrint('[ViewModel] Stats: Frames=$_totalFrames, Face=$faceRate%, Emotion=$emotionRate%');
    }
  }
